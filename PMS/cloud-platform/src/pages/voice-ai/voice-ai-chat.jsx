...
const QuickActions = ({ onAction }) => {
  return (
    <div className="flex flex-wrap items-center justify-center gap-4 mt-8 -mx-24">
      <Text letterSpacing="wider">Try:</Text>
...
const CharacterSelector = ({ selectedCharacter, onSelectCharacter }) => {
  ...
  const handleNext = () => {
    setStartIndex((prev) =>
      Math.min(characters.length - ITEMS_PER_PAGE, prev + 1),
    );
  };
  ...
  return (
    <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50">
      <div className="flex items-center gap-6 px-10 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 shadow-2xl relative">
        <button
          onClick={handlePrev}
          disabled={startIndex === 0}
          className={`text-[hsl(var(--primary))] hover:text-white transition-colors ${startIndex === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          ...
          {visibleCharacters.map((char, i) => {
            ...
            return (
              <motion.button
                ...
                className={`flex items-center gap-3 group py-4 transition-all ${
                  isActive ? "opacity-100" : "opacity-40 hover:opacity-100"
                }`}
              >
                <div
                  className={`relative p-0.5 rounded-full ${isActive ? "bg-gradient-to-tr from-[hsl(var(--primary))] to-[hsl(var(--accent))] shadow-glow-cyan" : ""}`}
                >
                  <Avatar.Root
                    size={{
                      base: "2xs",
                ...
                <span
                  className={`text-sm font-medium tracking-wide ${isActive ? "text-white" : "text-[hsl(var(--foreground)/0.6)]"}`}
                >
                  {char.name}
                </span>
              </motion.button>
        ...
        <button
          ...
          className={`text-[hsl(var(--primary))] hover:text-white transition-colors ${startIndex + ITEMS_PER_PAGE >= characters.length ? "opacity-50 cursor-not-allowed" : ""}`}
        >
...
const VoiceAiChat = () => {
  ...
  const [isLiveKitMode, setIsLiveKitMode] = useState(true);
